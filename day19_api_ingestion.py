...
def normalize_ticket(api_ticket):
    return {
        "id": api_ticket["id"],
        "text": f"{api_ticket['title']} {api_ticket['body']}"
    }

def load_tickets():
    api_tickets = fetch_tickets_from_api()
    return [normalize_ticket(ticket) for ticket in api_tickets]
...
with open("classified_tickets_api.json", "w") as file:
